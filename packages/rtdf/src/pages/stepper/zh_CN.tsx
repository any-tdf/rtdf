...
import { Stepper, Toast, Slider } from '../../lib/components';
...
function StepperZh() {
	...
	return (
		<div>
			...
			<div className='mx-4 mt-8 text-lg font-bold'>设置最大值/最小值/初始值</div>
			<div className='px-4 py-4'>
				<Stepper min={2} max={10} value={5} />
			</div>

			<div className='mx-4 mt-8 text-lg font-bold'>设置步长</div>
			<div className='px-4 py-4'>
				<Stepper step={5} />
			</div>

			<div className='mx-4 mt-8 text-lg font-bold'>强调数字区域</div>
			<div className='px-4 py-4'>
				<Stepper numberHighlight />
			</div>

			<div className='mx-4 mt-8 text-lg font-bold'>强调区域不用主题色</div>
			...
			<div className='mx-4 mt-8 text-lg font-bold'>竖向</div>
			...
			<div className='mx-4 mt-8 text-lg font-bold'>固定宽度</div>
			<div className='space-x-8 px-2 py-4'>
				<Stepper width={160} />
				<Stepper width={80} vertical />
			</div>

			<div className='mx-4 mt-8 text-lg font-bold'>格式化显示数字</div>
			<div className='px-4 py-4'>
				<div className='mb-2 text-sm'>保留一位小数</div>
				<Stepper decimal={1} step={0.1} max={1} min={0.1} value={0.5} />
			</div>
			<div className='px-4 py-4'>
				<div className='mb-2 text-sm'>保留四位小数</div>
				<Stepper decimal={4} step={0.0001} max={1} min={0.0001} value={0.5} />
			</div>

			<div className='mx-4 mt-8 text-lg font-bold'>异步显示数字</div>
			<div className='px-4 py-4'>
				<div className='mb-2 text-sm'>Toast 显示异步状态</div>
				<Stepper async={loading1} value={asyncValue1} onIncrease={() => handleChange1('increase')} onDecrease={() => handleChange1('decrease')} />
			</div>
			<Toast visible={loading1} type='loading' message='保存中...' onClose={() => setLoading1(false)} />
			<div className='mb-2 px-4 text-sm'>内部显示异步状态</div>
			<div className='flex gap-2 px-4 py-4'>
			...
			<div className='mx-4 mt-8 text-lg font-bold'>外部无 padding</div>
			<div className='px-4 py-2'>
			...
			<div className='mx-4 mt-8 text-lg font-bold'>不同位置注入 Class</div>
			<div className='px-4 py-2'>
