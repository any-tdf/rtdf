...
const ModeSwitch = () => {
	const { lang, themeMode, setThemeMode, sysTheme } = useAppContext();
	const isZh = lang === 'zh_CN';

	const label = useMemo(() => {
		if (themeMode === 'light') return isZh ? '亮模式' : 'Light';
		if (themeMode === 'dark') return isZh ? '暗模式' : 'Dark';
		return isZh ? '跟随系统' : 'System';
	}, [isZh, themeMode]);

	return (
		<div
			className="mx-auto flex justify-between rounded-sm bg-black/5 p-0.5 text-center dark:bg-white/10"
			aria-label={label}
		>
			<button
				onClick={() => setThemeMode('light')}
				className={`min-h-full cursor-pointer rounded-sm px-1 transition-all duration-150 ${themeMode === 'light' ? 'bg-white/80 shadow-xs' : ''}`}
				style={{
					color: sysTheme === 'dark' ? (themeMode === 'light' ? '#ff3f2e' : '#646464') : themeMode === 'light' ? '#ff3f2e' : themeMode === 'dark' ? '#646464' : '#cecece'
				}}
			...
			<button
				...
				className={`min-h-full cursor-pointer rounded-sm px-1 transition-all duration-150 ${themeMode === 'dark' ? 'bg-black/50 shadow-xs' : ''}`}
				style={{
					color: sysTheme === 'light'
						? themeMode === 'dark'
							? '#fff'
							: themeMode === 'auto'
								? '#cecece'
				...
				type="button"
			>
				<svg width="20" height="20" viewBox="0 0 20 20" style={{ display: 'inline', position: 'relative', top: '-1px' }} xmlns="http://www.w3.org/2000/svg">
					<path
						fill="currentColor"
			...
			<button
				...
				className={`min-h-full cursor-pointer rounded-sm px-1 transition-all duration-150 ${themeMode === 'auto' ? (sysTheme === 'light' ? 'bg-white/80 shadow-xs' : 'bg-black/50') : ''}`}
				style={{
					color:
						themeMode === 'light'
							? '#cecece'
