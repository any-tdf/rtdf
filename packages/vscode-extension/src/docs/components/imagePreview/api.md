...
| mask           | [`Mask`](/components?nav=mask&tab=1) | `{}`         | N    | 遮罩层参数。                                       |
| icon           | [`Icon`](/components?nav=icon&tab=1) | `{}`         | N    | 关闭图标参数。                                     |
| showRotation   | `boolean`                                               | `false`      | N    | 是否显示旋转按钮。                                 |
| rotationIcon   | [`Icon`](/components?nav=icon&tab=1) | `{}`         | N    | 旋转图标参数。                                     |
...
| onchange | `(index: number) => void`                        | index: 当前索引     | 切换图片时触发。 兼容 STDF 的小写事件别名。 |
| onClose     | `() => void`                                     | -                   | 关闭时触发。         |
| onclose | `() => void`                                     | -                   | 关闭时触发。 兼容 STDF 的小写事件别名。 |
| onScale     | `(scale: number) => void`                        | scale: 缩放比例     | 缩放时触发。         |
| onscale | `(scale: number) => void`                        | scale: 缩放比例     | 缩放时触发。 兼容 STDF 的小写事件别名。 |
| onRotate    | `(rotation: 0 \| 90 \| 180 \| 270) => void`      | rotation: 旋转角度  | 旋转时触发。         |

## ImagePreview Children

| 名称         | 类型                                                                | 参数                     | 说明               |
| ------------ | ------------------------------------------------------------------- | ------------------------ | ------------------ |
| children     | ReactNode | item, index              | 自定义内容。       |
| loadingChild | ReactNode | -                        | 自定义加载中。     |
| errorChild   | ReactNode | -                        | 自定义加载失败。   |
| indexChild   | ReactNode | current, total           | 自定义索引显示。   |
