...
The RTDF icon component's `type` defaults to `symbol`, which uses SVG Sprites technology to display icons. This reduces HTTP requests and improves page performance.

RTDF's SVG Sprites use [SVG symbol](https://developer.mozilla.org/en-US/docs/Web/SVG/Element/symbol). The principle is similar to CSS Sprite technology, where SVG files in the project are concatenated into a single file, and the corresponding icon is displayed through the use element in SVG.
...
RTDF uses [@any-tdf/vite-plugin-svg-symbol](https://www.npmjs.com/package/@any-tdf/vite-plugin-svg-symbol) for merging SVG files into SVG Sprites. Due to the diversity of SVG formats, there may be cases where the plugin doesn't process accurately. Please submit an issue on [GitHub](https://github.com/any-tdf/rtdf/issues) with specific details about the SVG file.
...
RTDF can also use icons via [Iconify](https://iconify.design) by setting `type` to `iconify` or `iconify-color`.

Iconify has many usage methods. RTDF chooses the Tailwind 4 plugin approach, which has minimal code intrusion. Refer to [Iconify for Tailwind CSS](https://iconify.design/docs/usage/css/tailwind/tailwind4). The usage in RTDF is as follows:
...
Some RTDF components use built-in SVG icons internally, such as `Alert`, `Toast`, `Checkbox`, `Radio`, and `Pagination`. The default built-in icon library is `remix`. You can also switch to `lucide`, `phosphor`, `tabler`, `iconoir`, or `reicon`. To switch those internal component icons, set `builtInIconLibrary` on `ConfigProvider`.
...
Generally, there are several ways to use SVG icons in RTDF projects.
...
2. Use the [@any-tdf/vite-plugin-svg-symbol](https://www.npmjs.com/package/@any-tdf/vite-plugin-svg-symbol) plugin. Merge SVG files into SVG symbol after compilation, but the SVG files must conform to the standard, otherwise they may not be merged correctly. **Recommended**.
