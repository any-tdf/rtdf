...
| loading      | [`Loading`](/components?nav=loading&tab=1) | `{}`     | N    | 加载组件参数。                             |
| icon         | [`Icon`](/components?nav=icon&tab=1)     | `{}`       | N    | 上传图标参数。                             |
| deleteIcon   | [`Icon`](/components?nav=icon&tab=1)     | `{}`       | N    | 删除图标参数。                             |
...
| onadd | `(files: File[]) => void`                                | files: 文件列表               | 添加文件时触发。 兼容 STDF 的小写事件别名。 |
| onDelete    | `(item: ImageListItemProps, index: number) => void`      | item: 图片项，index: 索引     | 删除时触发。             |
| ondelete | `(item: ImageListItemProps, index: number) => void`      | item: 图片项，index: 索引     | 删除时触发。 兼容 STDF 的小写事件别名。 |
| onRetry     | `(item: ImageListItemProps, index: number) => void`      | item: 图片项，index: 索引     | 重试时触发。             |
| onretry | `(item: ImageListItemProps, index: number) => void`      | item: 图片项，index: 索引     | 重试时触发。 兼容 STDF 的小写事件别名。 |
| onPreview   | `(item: ImageListItemProps, index: number) => void`      | item: 图片项，index: 索引     | 预览时触发。             |
| onpreview | `(item: ImageListItemProps, index: number) => void`      | item: 图片项，index: 索引     | 预览时触发。 兼容 STDF 的小写事件别名。 |
| onExceed    | `(files: File[], max: number) => void`                   | files: 文件列表，max: 最大数量 | 超出数量限制时触发。     |
| onexceed | `(files: File[], max: number) => void`                   | files: 文件列表，max: 最大数量 | 超出数量限制时触发。 兼容 STDF 的小写事件别名。 |
| onOversized | `(file: File, maxSize: number) => void`                  | file: 文件，maxSize: 最大大小 | 超出大小限制时触发。     |
| onoversized | `(file: File, maxSize: number) => void`                  | file: 文件，maxSize: 最大大小 | 超出大小限制时触发。 兼容 STDF 的小写事件别名。 |
| onClickUpload | `() => void`                                           | -                             | 点击上传按钮时触发。     |
| onclickUpload | `() => void`                                           | -                             | 点击上传按钮时触发。 兼容 STDF 的小写事件别名。 |
...
| 名称        | 类型                                                                | 参数         | 说明               |
| ----------- | ------------------------------------------------------------------- | ------------ | ------------------ |
| uploadChild | ReactNode | -            | 自定义上传按钮。   |
| itemChild   | ReactNode | item, index  | 自定义每项。       |
| statusChild | ReactNode | item         | 自定义状态显示。   |
