...
> Default parameters use `src/lib/symbol` and `public/fonts`. If you use a custom static directory layout, please modify the configuration in vite.config.js or vite.config.ts.
...
This configuration will combine two symbols. **When using the RTDF Icon component, please update `path` to the corresponding `fonts/symbol.svg` or `fonts/icon.svg`**.
