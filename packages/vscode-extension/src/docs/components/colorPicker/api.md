...
| showPanel    | `boolean`                                                         | `true`                    | N    | 是否显示颜色面板。                       |
...
| popup        | [`Popup`](/components?nav=popup&tab=1)\|`null` | `{}`                      | N    | 弹出层参数，`null` 时直接展示。          |
| tab          | [`Tab`](/components?nav=tabs&tab=1)            | `{}`                      | N    | Tab 配置。                               |
...
| onchange | `(colors: string[]) => void` | colors - 当前颜色值数组（各模式格式） | 颜色变化时触发（直接显示模式）。 兼容 STDF 的小写事件别名。 |
| onClose  | `(colors: string[]) => void` | colors - 当前颜色值数组（各模式格式） | 关闭时触发（Popup 模式）。             |
| onclose | `(colors: string[]) => void` | colors - 当前颜色值数组（各模式格式） | 关闭时触发（Popup 模式）。 兼容 STDF 的小写事件别名。 |
| onCopy   | `(text: string) => void`    | text - 复制的文本                    | 复制颜色值时触发。                     |
| oncopy | `(text: string) => void`    | text - 复制的文本                    | 复制颜色值时触发。 兼容 STDF 的小写事件别名。 |
