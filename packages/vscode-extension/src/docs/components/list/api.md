...
| clickable       | `boolean`                                            | `true`        | N    | 列表项是否显示为可点击状态。                   |
...
| onbatchChange | `(selected: (string\|number)[]) => void`                                | `selected` - 当前选中项 key 数组               | 批量选择项变化时触发。 兼容 STDF 的小写事件别名。 |
| onBatchModeChange | `(batchMode: boolean) => void`                                      | `batchMode` - 当前批量选择模式                 | 批量选择模式变化时触发。       |
| onClickItem   | `(item: T, index: number) => void`                                       | `item` - 当前项<br />`index` - 当前索引        | 点击列表项时触发。             |
| onclickItem | `(item: T, index: number) => void`                                       | `item` - 当前项<br />`index` - 当前索引        | 点击列表项时触发。 兼容 STDF 的小写事件别名。 |
...
| onswipeAction | `(actionIndex: number, action: SwipeActionProps, item: T, itemIndex: number) => void` | `actionIndex` - 操作索引<br />`action` - 操作配置<br />`item` - 当前项<br />`itemIndex` - 当前索引 | 点击滑动操作按钮时触发。 兼容 STDF 的小写事件别名。 |

## List Children

| 名称       | 类型                                                                | 参数               | 说明                     |
| ---------- | ------------------------------------------------------------------- | ------------------ | ------------------------ |
| itemChild  | ReactNode | `(item: T, index)` | 列表项内容（必传）。     |
| headerChild| ReactNode | -                  | 自定义列表头部内容。     |
| footerChild| ReactNode | -                  | 自定义列表底部内容。     |
...
| onclick | `() => void`                      | -        | 点击按钮时触发的回调。 兼容 STDF 的小写事件别名。 |
