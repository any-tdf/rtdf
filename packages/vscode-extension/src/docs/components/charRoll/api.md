...
| easing | EasingProps | `'cubicOut'` | N | 缓动函数，共 31 个值，RTDF 内置 easing 函数集。 |
...
| onstart | `() => void` | - | 动画开始时触发。 兼容 STDF 的小写事件别名。 |
| onComplete | `() => void` | - | 动画完成时触发。 |
| oncomplete | `() => void` | - | 动画完成时触发。 兼容 STDF 的小写事件别名。 |
| onChange | `(value: string) => void` | value: 当前值 | 值变化时触发。 |
| onchange | `(value: string) => void` | value: 当前值 | 值变化时触发。 兼容 STDF 的小写事件别名。 |

## CharRoll Children

| 名称 | 类型 | 参数 | 说明 |
| --- | --- | --- | --- |
| children | ReactNode | char: string, index: number | 自定义字符渲染。 |
