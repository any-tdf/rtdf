...
设置 `autoStart` 为 `false` 可以禁用自动开始。通过 `ref` 获取组件实例后，可以调用以下方法手动控制动画：
...
滚动动画的缓动类型由 `easing` 属性决定，默认值为 `cubicOut`，可选值有 31 个（一个线性动画 `linear` 与下面 30 种非线性动画），具体值可参考下表。动画持续时间由 `duration` 属性决定。
...
通过 `children` render props 可以完全自定义每个字符的渲染内容。render props 接收两个参数：
