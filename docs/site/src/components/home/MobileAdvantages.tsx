...
const MobileAdvantages = () => {
	...
	const gridRef = useStaggerChildren<HTMLDivElement>({ stagger: 80, duration: 600 });
	const noteRef = useFadeInUp<HTMLDivElement>({ delay: 400, duration: 800, distance: 20 });
			...
			<div className="mx-auto max-w-6xl px-4">
				...
				<div ref={gridRef} className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
					{mobileFeatures.map((feature) => (
						<div
							key={feature.title}
							className="group rounded-2xl bg-white/50 p-6 backdrop-blur-sm transition-all duration-300 hover:bg-white/80 dark:bg-gray-800/50 dark:hover:bg-gray-800/80"
						>
							<div className="mb-4 flex size-14 items-center justify-center rounded-2xl bg-primary/10 transition-transform duration-300 group-hover:scale-110 dark:bg-dark/10">
								<svg className="size-7 text-primary dark:text-dark" viewBox="0 0 24 24" fill="currentColor">
									<path d={iconPaths[feature.icon]} />
								</svg>
							</div>
							<h3 className="mb-2 text-lg font-semibold text-gray-900 dark:text-gray-100">{feature.title}</h3>
							<p className="mb-4 text-sm leading-relaxed opacity-60">{feature.desc}</p>
							<div className="flex flex-wrap gap-2">
								{feature.details.map((detail) => (
									<span
										key={detail}
										className="rounded-full bg-primary/5 px-2 py-1 text-xs text-primary dark:bg-dark/10 dark:text-dark"
									>
										{detail}
									</span>
								))}
				...
				<div ref={noteRef} className="mt-8 text-center">
					<p className="text-sm opacity-60">
						{isZh ? '真机测试，iOS 和 Android 表现一致' : 'Real device tested, consistent on iOS and Android'}
					</p>
				</div>
			</div>
		</section>
