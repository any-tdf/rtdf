...
| onchange | `(current: number) => void` | current - 当前页码 | 页码变化时触发。 兼容 STDF 的小写事件别名。 |
| onNext   | `(current: number) => void` | current - 当前页码 | 点击下一页时触发。 |
| onnext | `(current: number) => void` | current - 当前页码 | 点击下一页时触发。 兼容 STDF 的小写事件别名。 |
| onPre    | `(current: number) => void` | current - 当前页码 | 点击上一页时触发。 |
| onpre | `(current: number) => void` | current - 当前页码 | 点击上一页时触发。 兼容 STDF 的小写事件别名。 |
