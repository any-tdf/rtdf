...
| popup           | [`Popup`](/components?nav=popup&tab=1) \| `null` | `{}`                                 | N    | 弹出层参数，传 null 时不使用弹出层。         |
| button          | [`Button`](/components?nav=button&tab=1) | `{}`                                 | N    | 确定按钮参数。                               |
| card            | [`Card`](/components?nav=card&tab=1)     | `{}`                                 | N    | 月份卡片参数，monthCard 为 true 时生效。     |
...
| onconfirm | `(dates: string[]) => void` | dates - 所有选定日期格式化后字符串组成的数组。 | 点击确定按钮时触发。 兼容 STDF 的小写事件别名。 |
