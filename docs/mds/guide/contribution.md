...
调试 RTDF 必须要有个工程环境，为此提供了一个简单的 Demo 工程。RTDF 本身不受 Node 版本限制，Demo 使用 Vite 开发，所以请确保你的 Node.js 版本 >= 14.18+，参考 [Vite 官网](https://cn.vitejs.dev/guide/#scaffolding-your-first-vite-project)。
...
- [ES7+ React/Redux/React-Native snippets](https://marketplace.visualstudio.com/items?itemName=dsznajder.es7-react-js-snippets)：React 开发插件。
...
启动成功之后，可在浏览器中打开 `http://localhost:8887` 查看效果，请将浏览器的开发者工具切换为移动端模式。

可对 `packages/rtdf/src/lib/components` 目录下组件源码进行修改并实时查看修改效果。注意，VS Code 与 WebStorm 都支持在调用组件时直接跳转到组件源码。

如果不想本地开发，可以选择使用 StackBlitz 在线简单调试。直接打开 [RTDF Demo](https://stackblitz.com/github/any-tdf/demo-rtdf) 即可开始在线调试。
...
- 克隆你的仓库至本地。
- 修改组件源码并验证通过。
- 「可选」补充此次修改的中英文档中，文档位于 `docs/mds/components` 目录下。根据具体修改内容可能需要修改 api、FAQ、guide、version 等文档。对 version 的修改请增加 tag，具体参考 [RTDF Version tag](https://github.com/any-tdf/rtdf/blob/main/docs/mds/components/button/version.md?plain=1)。
- 到仓库 `packages/rtdf` 目录，执行 `pnpm i` 或 `npm i` 或 `bun i` 安装依赖。
