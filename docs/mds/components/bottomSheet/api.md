...
| iconRadius      | `'none'\|'xs'\|'sm'\|'md'\|'lg'\|'xl'\|'2xl'\|'full'\|''` | `''`                         | N    | 返回和关闭图标按钮的圆角风格。     |
...
| onclose | `() => void`               | -                   | 浮窗关闭时触发。 兼容 STDF 的小写事件别名。 |
| onClickMask    | `() => void`               | -                   | 点击浮窗遮罩时触发，即使 maskClosable 为 false 也会触发。 |
| onclickMask | `() => void`               | -                   | 点击浮窗遮罩时触发，即使 maskClosable 为 false 也会触发。 兼容 STDF 的小写事件别名。 |
| onBack         | `() => void`               | -                   | 点击左侧返回图标时触发。                                  |
| onback | `() => void`               | -                   | 点击左侧返回图标时触发。 兼容 STDF 的小写事件别名。 |
| onHeightChange | `(height: number) => void` | height - 当前高度。 | 滑动结束时触发。                                          |
| onheightChange | `(height: number) => void` | height - 当前高度。 | 滑动结束时触发。 兼容 STDF 的小写事件别名。 |
