...
const StatCounter = () => {
	...
	const componentCount = useMemo(() => menuList.reduce((total, category) => total + category.childs.length, 0), []);
	...
	const performanceAdvantages = useMemo(
		() => [
			{
				title: isZh ? '渲染优化' : 'Render Optimized',
				desc: isZh ? '合理更新，提升渲染效率' : 'Efficient updates for rendering',
			...
			},
			{
				...
				desc: isZh ? '减少依赖，体积更小' : 'Fewer deps, smaller bundle',
			...
			},
			{
				...
				icon: 'M5 3V19H21V21H3V3H5ZM20.293 6.293L21.707 7.707L16 13.414L13 10.414L8.707 14.707L7.293 13.293L13 7.586L16 10.586L20.293 6.293Z',
		...
		const observer = new IntersectionObserver(
			(entries) => {
				...
				const animate = (now: number) => {
					...
					setCounts(stats.map((stat) => Math.floor(easeOut * stat.value)));
					if (progress < 1) {
	...
	return (
		<section id="stat-counter" ref={sectionRef} className="relative overflow-hidden px-4 py-20 md:px-12 md:py-32">
			<div className="relative z-10 mx-auto max-w-6xl">
				<div className="mb-12 text-center">
					...
					<p className="mx-auto max-w-2xl text-base opacity-70">
						{isZh ? '组件丰富、主题多样、国际化完善、类型安全' : 'Rich components, diverse themes, comprehensive i18n, and full type safety'}
					</p>
				</div>

				<div className="mb-12 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4 md:gap-6">
					{stats.map((stat, index) => (
						<div key={stat.label} className="relative overflow-hidden rounded-2xl p-5 shadow-lg transition-all active:scale-95">
							<div className={`absolute inset-0 bg-linear-to-br ${stat.color}`}></div>
							<svg className="absolute right-2 top-2 size-16 text-white opacity-20 md:size-24" viewBox="0 0 24 24" fill="currentColor">
								<path d={stat.icon} />
							</svg>
							<div className="relative z-10">
								...
								<div className="mt-1 text-xs text-white/70">{stat.desc}</div>
							</div>
						</div>
					))}
				</div>
				...
				<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
					{[...performanceAdvantages, ...rtdfFeatures].map((item) => (
						<div
							key={item.title}
							className="flex flex-col items-center rounded-xl border border-gray-200/50 bg-white/80 p-4 text-center shadow-lg backdrop-blur-sm transition-all active:scale-95 dark:border-white/10 dark:bg-gray-900/80"
						>
							<div className={`mb-3 flex size-10 shrink-0 items-center justify-center rounded-lg bg-linear-to-br ${item.color}`}>
