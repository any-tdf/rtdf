...
function ButtonGroupEn() {
	...
	const basicItems = useMemo<ButtonGroupItemProps[]>(
		() => [
			...
			{ text: 'Share', icon: { name: 'ri-share-forward-line', size: 18 }, onClick: () => setVisible(true) },
		],
		[],
	);

	const textOnlyItems = useMemo<ButtonGroupItemProps[]>(() => [{ text: 'Cancel' }, { text: 'Confirm', onClick: () => setVisible(true) }], []);
	...
	const disabledItems = useMemo<ButtonGroupItemProps[]>(() => [{ text: 'Enabled' }, { text: 'Disabled', disabled: true }, { text: 'Enabled' }], []);

	const textIconAndIconItems = useMemo<ButtonGroupItemProps[]>(() => [{ text: 'Search', icon: { name: 'ri-search-line', size: 18 } }, { icon: { name: 'ri-filter-line', size: 20 } }], []);

	const iconAndTextIconItems = useMemo<ButtonGroupItemProps[]>(
		() => [{ icon: { name: 'ri-arrow-left-line', size: 20 } }, { text: 'Next', icon: { name: 'ri-arrow-right-line', size: 18 }, iconPosition: 'right' }],
		[],
	);
	...
	const complexItems = useMemo<ButtonGroupItemProps[]>(
		() => [{ icon: { name: 'ri-skip-back-line', size: 18 } }, { text: 'Play', icon: { name: 'ri-play-line', size: 18 } }, { icon: { name: 'ri-skip-forward-line', size: 18 } }],
		[],
	);
	...
	return (
		<div className='flex flex-col space-y-8 py-8'>
			...
			<div>
				<div className='p-4 font-bold'>Two-button combinations</div>
			...
			<div>
				<div className='p-4 font-bold'>Icon position</div>
				<ButtonGroup items={iconPositionItems} />
				<ButtonGroup items={iconPositionItems} fill='lineState' />
				<ButtonGroup items={iconPositionItems} fill='colorLight' />
			</div>
			<div>
				<div className='p-4 font-bold'>Complex combination</div>
			...
			<div>
				<div className='p-4 font-bold'>Different fill modes</div>
			...
			<div>
				<div className='p-4 font-bold'>fill + state combination</div>
			...
			<div>
				<div className='p-4 font-bold'>Different radius styles</div>
				<div className='px-4 pb-4'>
					<Slider value={radiusIndex} minRange={0} maxRange={7} step={1} showSteps stepLabels={radiusLabels} onChange={(v: number) => setRadiusIndex(v)} />
				</div>
			...
			<div>
				<div className='p-4 font-bold'>Different sizes</div>
			...
			<div>
				<div className='p-4 font-bold'>Divider height</div>
			...
			<div>
				<div className='p-4 font-bold'>Border style</div>
			...
			<Toast visible={visible} message='Button clicked!' onClose={() => setVisible(false)} />
		</div>
	);
}

export default ButtonGroupEn;
